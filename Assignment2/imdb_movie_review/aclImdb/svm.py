...
data_train = np.ndarray(shape=(len(list_train_pos)+len(list_train_neg),300))
...
data_test_pos = np.ndarray(shape=(len(list_test_pos),300))
for i in range(0,len(list_test_pos)):
	data_test_pos[i] = list_test_pos[i]

data_test_neg = np.ndarray(shape=(len(list_test_neg),300))
