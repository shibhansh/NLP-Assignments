...
if [ ! -e "$trdir"/en-generation.hfst.ol ] ; then
    echo "$trdir/en-generation.hfst.ol not found"
    exit 1
fi
if [ "$1" == "--help" ] ; then
...
if [ $proc_present -eq 0 ] ; then
    ...
    if [ $# -lt 1 ] ; then
	hfst-proc --xerox "$trdir"/en-generation.hfst.ol
    elif [ $# -lt 2 ] ; then
	case $1 in
            cg,CG)
		hfst-proc --cg "$trdir"/en-generation.hfst.ol ;;
            apertium)
		hfst-proc --apertium "$trdir"/en-generation.hfst.ol ;;
            xerox)
		hfst-proc --xerox "$trdir"/en-generation.hfst.ol ;;
            *)
	...
	case $1 in
            cg,CG)
		hfst-proc --cg "$trdir"/en-generation.hfst.ol $2 ;;
            apertium)
		hfst-proc --apertium "$trdir"/en-generation.hfst.ol $2 ;;
            xerox)
		hfst-proc -x "$trdir"/en-generation.hfst.ol $2 ;;
            *)
		echo Unknown output format $1, using xerox
		hfst-proc -x "$trdir"/en-generation.hfst.ol $2 ;;
	esac
    ...
    if [ $# -lt 1 ] ; then
	hfst-optimized-lookup "$trdir"/en-generation.hfst.ol
    else
	if [ $# -gt 3 ] ; then
	    echo 1>&2 "Ignoring $3..., for multiple file analysis use cat"
	fi
	hfst-optimized-lookup "$trdir"/en-generation.hfst.ol < $2
    fi
elif [ $java_optlookup_present -eq 0 ] ; then
    echo 1>&2 Using hfst-optimized-lookup-java...
    if [ $# -lt 1 ] ; then
	java HfstOptimizedLookup "$trdir"/en-generation.hfst.ol
    else
	if [ $# -gt 3 ] ; then
	    echo 1>&2 "Ignoring $3..., for multiple file analysis use cat"
	fi
	java HfstOptimizedLookup "$trdir"/en-generation.hfst.ol < $2
    fi
else
