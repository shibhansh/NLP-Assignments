...
if [ ! -e "$trdir"/en-analysis.hfst.ol ] ; then
    echo "$trdir/en-analysis.hfst.ol not found"
    exit 1
fi
if [ "$1" == "--help" ] ; then
...
if [ $proc_present -eq 0 ] ; then
    ...
    if [ $# -lt 1 ] ; then
	hfst-proc -x "$trdir"/en-analysis.hfst.ol
    elif [ $# -lt 2 ] ; then
	case $1 in
	    cg,CG)
		hfst-proc --cg "$trdir"/en-analysis.hfst.ol ;;
	    apertium)
		hfst-proc --apertium "$trdir"/en-analysis.hfst.ol ;;
	    xerox)
		hfst-proc -x "$trdir"/en-analysis.hfst.ol ;;
	    *)
	...
	case $1 in
	    cg,CG)
		hfst-proc --cg "$trdir"/en-analysis.hfst.ol $2 ;;
	    apertium)
		hfst-proc --apertium "$trdir"/en-analysis.hfst.ol $2 ;;
	    xerox)
		hfst-proc -x "$trdir"/en-analysis.hfst.ol $2 ;;
	    *)
		echo Unknown output format $1, using xerox
		hfst-proc -x "$trdir"/en-analysis.hfst.ol $2 ;;
	esac
    ...
    if [ $# -lt 1 ] ; then
	hfst-optimized-lookup "$trdir"/en-analysis.hfst.ol
    else
	if [ $# -gt 3 ] ; then
	    echo 1>&2 "Ignoring $3..., for multiple file analysis use cat"
	fi
	hfst-optimized-lookup "$trdir"/en-analysis.hfst.ol < $2
    fi
elif [ $java_optlookup_present -eq 0 ] ; then
    echo 1>&2 Using hfst-optimized-lookup-java...
    if [ $# -lt 1 ] ; then
	java HfstOptimizedLookup "$trdir"/en-analysis.hfst.ol
    else
	if [ $# -gt 3 ] ; then
	    echo 1>&2 "Ignoring $3..., for multiple file analysis use cat"
	fi
	java HfstOptimizedLookup "$trdir"/en-analysis.hfst.ol < $2
    fi
else
